...
class AttentionModule(nn.Module):
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        scores = self.V_att(scores)
        alpha = torch.nn.functional.softmax(scores, dim=0)
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        context = torch.zeros(1,dim)
        for i in range (seq):
            context = context + alpha[i] * encoder_out[i]
...
def greedyDecoder(decoder, encoder_out, encoder_hidden, maxLen,
    ...
    for t in range(maxLen):
        ...
        #if index == 2:
        #    output[0][0][7] = output[0][0][7] *0.8
        #    output[0][0][20] = output[0][0][20] *0.8
        #    output[0][0][51] = output[0][0][51] *0.8
        #    output[0][0][96] = output[0][0][96] *0.8
        output[0][0][index] = output[0][0][index] * 0.8
        outputs[t] = output
        alphas[t] = alpha.data
        output = torch.autograd.Variable(output.data.max(dim=2)[1])
        if int(output.data) == eos_index:
            break
    return outputs, alphas.permute(1, 0, 2)

def beamSearch(decoder, encoder_out, encoder_hidden, maxLen,
                  eos_index):
    seq1_len, batch_size, _ = encoder_out.size()
    target_vocab_size = decoder.target_vocab_size
    ...
    candidates = torch.zeros(beam_size*(beam_size+1), dtype = float)
    tracker = torch.zeros(beam_size*(beam_size+1), maxLen+1,dtype = int)
    for i in range(beam_size*(beam_size+1)):
        tracker[i][0] = 0
        for j in range(1,maxLen+1):
            tracker[i][j] = eos_index
    candidates[0] = output[0]
    #k = outputs.shape[2]
    
    outputs = torch.zeros(beam_size,1,1, dtype = int)
    outputs[0][0][0] = 2
    
    decoder_hiddens = torch.zeros(beam_size*2,decoder_hidden.shape[0],decoder_hidden.shape[1],decoder_hidden.shape[2])
    decoder_hiddens[0] = decoder_hidden
    
    alphas = torch.zeros(beam_size*2,maxLen, batch_size, seq1_len)

    #for t in range(maxLen*k*beam_size):
    while(1):
        for j in range(beam_size):
            output, decoder_hidden, alpha = decoder(
                outputs[j], encoder_out, decoder_hiddens[j])
            ...
            #    output[0][0][51] = output[0][0][51] *0.8
            #    output[0][0][96] = output[0][0][96] *0.8
            output[0][0][outputs[j][0][0]] = output[0][0][outputs[j][0][0]] * 0.8
            
            top = output.topk(beam_size, dim=2)
            #print(top)#############################################################################
            index = tracker[j][0]
            if outputs[j][0][0] == 2:
                candidates[beam_size*(j+1):beam_size+(beam_size*(j+1))] = top.values[0][0] * 0.01
            else:
                candidates[beam_size*(j+1):beam_size+(beam_size*(j+1))] = top.values[0][0] * candidates[j] * 0.01
            candidates[j] = 0.
            
            decoder_hiddens[beam_size+j] = decoder_hidden
            alphas[beam_size+j][index] = alpha
            
            for i in range(beam_size):
                tracker[beam_size*(j+1)+i] = tracker[j]
                tracker[beam_size*(j+1)+i][index+1] = top.indices[0][0][i]
                tracker[beam_size*(j+1)+i][0] = index + 1
            if outputs[j][0][0] == 2:
                #print("sos")
                break
        sorting = torch.sort(candidates, descending = True)
        candidates = sorting.values
        temp = torch.zeros(beam_size*(beam_size+1), maxLen+1,dtype = int)
        for i in range(beam_size*(beam_size+1)):
            temp[i] = tracker[sorting.indices[i]]
        tracker = temp
        

        
        for j in range(beam_size):
            outputs[j][0][0] = tracker[j][tracker[j][0]]
            decoder_hiddens[j] = decoder_hiddens[int(sorting.indices[j]/beam_size)+beam_size-1]
            alphas[j] = alphas[int(sorting.indices[j]/beam_size)+beam_size-1]
            
        
            output_res = torch.autograd.Variable(outputs[j])
            #print(outputs)
            if int(output_res.data) == eos_index or tracker[j][0] > maxLen-1:
                return tracker[j][1:], alphas[j].permute(1, 0, 2)
        #print(candidates)##############################################################
        #print(tracker)##################################################################
        #print(sorting.indices)##########################################
        #print(decoder_hiddens)#######################################

def translate(srcFile_path, model, test_iter):
    results = []
    ...
    for i, batch in tqdm(enumerate(test_iter)):
        ...
        #output = model.tgt2txt(output.data).strip().split('<EOS>')[0] #for beam search
        #print(output)
        #breakline
        
        # unknown words replacement
        src_ori_text = srcFile.readline().strip().split(' ')
        src_tokens = batch.src.tolist()
        src_unknown_pos = [i for i in range(len(src_tokens)) if src_tokens[i][0]==0]
        tgt_words = output.split(' ')
        tgt_unknown_pos = [i for i in range(len(tgt_words)) if tgt_words[i]=='<unk>']
        att_max = [i.index(max(i)) for i in attention.tolist()[0]]
        
        for unk in tgt_unknown_pos:
            if(src_tokens[att_max[unk]][0]==0):
                tgt_words[unk] = src_ori_text[att_max[unk]]
            else:
                single_word = torch.LongTensor([ src_tokens[att_max[unk]] ])
                
                single_out, _ = model(single_word)
                single_out = single_out.topk(1)[1]
                single_out = model.tgt2txt(single_out[:, 0].data[0]).strip().split('<EOS>')[0]
                #single_out = model.tgt2txt(single_out.data).strip().split('<EOS>')[0]
                if(single_out=='<unk>'):
                    tgt_words[unk] = src_ori_text[att_max[unk]]
                else:
                    tgt_words[unk] = single_out
        output = " ".join(tgt_words)
        #print(output)
        #breakline
...
def translate_temp(model, test_iter):
    results = []
    for i, batch in tqdm(enumerate(test_iter)):
        output, attention = model(batch.src)
        output = output.topk(1)[1]
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, maxLen=None, eos_index=2):
        ...
        return greedyDecoder(self.decoder, encoder_out, encoder_hidden,
                             maxLen, eos_index)
        #return beamSearch(self.decoder, encoder_out, encoder_hidden,
        #                     maxLen, eos_index)
...
if __name__ == '__main__':
    ...
    results = translate(opts.input, model, test_iter)
