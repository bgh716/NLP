...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, ix_to_word, clr):
        torch.manual_seed(1)
        ...
        self.lstm = nn.LSTM(embedding_dim+hidden_dim, hidden_dim, bidirectional=False)
        self.clr_lstm = nn.LSTM(clr, hidden_dim, bidirectional=False)
...
class LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64, clr=400):
        self.unk = unk
        ...
        self.model = LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix), self.ix_to_word, self.clr)
