...
class LexSub:
    ...
    def find_context(self, candidates, index, sentence, oldvec):
        ...
        for word in sentence[index:index+sentence_range+1]:
            if word in oldvec.wvecs_dict and word.isalpha() and word not in self.non_context:
                context_words.append(oldvec.wvecs_dict[word])
        ...
        if context_words:
            for word in candidates:
                candidates_dict[word] = ((1-spatial.distance.cosine(self.wvecs_dict[word], v)) + sum((1-spatial.distance.cosine(oldvec.wvecs_dict[word], c)) for c in context_words)) / (bal+1)
        else:
            return candidates[:10]
            
        return list(map(lambda k: k[0],sorted(candidates_dict.items(), key=lambda x: x[1], reverse = True)[:30]))
...
if __name__ == '__main__':
    ...
    lexsub = LexSub(opts.wordvecretrofile, "", "", int(opts.topn)) ## 10*n candidates
